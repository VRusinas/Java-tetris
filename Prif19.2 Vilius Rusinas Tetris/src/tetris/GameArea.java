...
public class GameArea extends JPanel 
{
    ...
    {
        Random r = new Random();
        block = allblocks[r.nextInt(allblocks.length)]; 
        block.spwanAboveGame(gridColumns);
    }
         ...
         for( int r = 0; r < gridRows; r++)
        {
            for( int c = 0; c < gridColumns; c++)
            {
               ...
               if(color != null)
               {
                   ...
                   drawShapes(g, color, x, y); 
               }
            }
        }
    }
    private void drawShapes(Graphics g, Color color, int x, int y ){
            g.setColor(color);
